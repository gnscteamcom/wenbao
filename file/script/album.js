...
function Album(id, s) {
	...
	/*for(i=0;i<3;i++)document.getElementById("topmobile1"+i).style.display="none";
	
	document.getElementById("topmobile1"+n).style.display="";
	document.getElementById("www"+n).style.backgroundColor="#187BBE";
    document.getElementById('tabMore').setAttribute('href', document.getElementById("www"+n).getAttribute('data'));
	for (i=1;i<=3;i++)
	{
		if (i!=n)
		{
			document.getElementById("www"+i).style.backgroundColor="";
		}

	}*/
	
	for(var i=0; i<3; i++) {Dd('x_x'+i).style.display = i==id ? '' : 'none';}
	//Dd('x_x').href = s;
}
function SAlbum() {
	s = Dd('mid_pic').src;
