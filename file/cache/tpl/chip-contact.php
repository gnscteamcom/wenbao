...
<?php if($member['validated']) { ?>
<br />&nbsp;<img src="<?php echo DT_SKIN;?>image/check_right.gif" align="absmiddle"/> 通过<?php echo $member['validator'];?>认证<?php } ?>
&nbsp;<!--<a href="<?php echo userurl($member['username'], 'file=credit');?>">[诚信档案]</a>-->
</li>
<?php } ?>
<li style="padding-top:6px;padding-bottom:6px;">
<span>用户名</span><?php echo $member['truename'];?>(<?php echo gender($member['gender']);?>)&nbsp;<?php echo $member['career'];?>&nbsp;
...
<li><span>信誉</span><b style="color:orange;"><?php echo $member['credit'];?></b></li>
...
<?php if($member['mobile']) { ?><li><span>手机</span><?php echo anti_spam($member['mobile']);?><?php if($DT['sms'] && $member['vmobile']) { ?>&nbsp;&nbsp;<a href="<?php echo $MODULE['2']['linkurl'];?>sms.php?action=add&auth=<?php echo encrypt($member['mobile']);?>" target="_blank" rel="nofollow">[发送短信]</a><?php } ?>
 ...
 -->
<!--<li><span>注册时间</span><?php echo date("Y-m-d",$member['regtime']); ?></li>-->
<li><span>最后登录</span><?php echo date("Y-m-d",$member['logintime']); ?></li>
