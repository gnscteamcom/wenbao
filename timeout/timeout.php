<?php
	...
	do {
		...
		//筛选出过期但状态还未改变的竞拍商品
		$now = time();
		$result = $db->query("SELECT itemid,totime,process,auction_user,price,margins FROM {$DT_PRE}auction WHERE totime!='' AND totime<".$now." AND process<2");
		while($row = $db->fetch_array($result)) {
			if ($row['auction_user']=='') {
				//如果竞拍商品到期后没有任何人竞拍，则自动归还锁定的保证金给卖家
				$update = "UPDATE {$DT_PRE}member SET bond = bond + 300, bondlocking = bondlocking - 300 WHERE username='".$row['username']."'";
				$db->query($update);
			}else{	
				$auctionList[] = $row;	
			}
