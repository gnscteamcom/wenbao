...
//判断保证金多少
	if($item['auction_zidong']<=5000){
		$bondauctionn=100;	
	}elseif($item['auction_zidong']>5000&&$item['auction_zidong']<=25000){
		$bondauctionn=$item['auction_zidong']*0.02;
	}elseif($item['auction_zidong']>25000){
		$bondauctionn=500;
	}
...
//判断保证金多少
	if($item['auction_price']<=5000){
		$bondauctionn=100;	
	}elseif($item['auction_price']>5000&&$item['auction_price']<=25000){
		$bondauctionn=$item['auction_price']*0.02;
	}elseif($item['auction_price']>25000){
		$bondauctionn=500;
	}
	$bondrsn=$db->query("select * from destoon_member where username='".$_username."'"); 
	while($bondrrn=mysql_fetch_array($bondrsn)){
...
////////////////////////////////////////////////////////////////////////	
	if ($item['marketprice'] <= $item['auction_price']) {
		...
		if($A1['num']>0 || $beyond==1){
			...
			//判断要冻结保证金多少
			if($zidong_totol<=5000){
				$bondauction=100;	
			}elseif($zidong_totol>5000&&$zidong_totol<=25000){
				$bondauction=$zidong_totol*0.02;
			}elseif($zidong_totol>25000){
				$bondauction=500;
			}
