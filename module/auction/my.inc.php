...
switch($action) {
	case 'add':
		...
		if($submit) {
			# 添加到提交成功后执行
			$rsaddb=$db->get_one("select * from destoon_member where username='".$_username."'");	
			$bond = $rsaddb['bond'];
			$bondlocking = $rsaddb['bondlocking'];
			if($bond>=300){
				$bondauctionadd=$bondlocking+300;
				$bondadd=$bond-300;
				$db->query("update destoon_member set bondlocking='".$bondauctionadd."',bond='".$bondadd."' where username='".$_username."'");

			}else{
				echo "<script>alert('保证金余额不足！');location.href='http://www.wenbaozaixian.com/member/my.php?mid=23&action=add';</script>";
	...
	default:
		...
		in_array($status, array(1, 2, 3, 4, 5,6)) or $status = 3;
		...
		if ($status == 5) {
			...
			$lists = $do->get_auction_list1($_username);
		}else{
...
if($_userid) {
	...
	$nums[5] = $rs['num']; 
	$rs = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE auction_user='$_username' AND status=3 AND auction_status=1");
	//print_r ("SELECT COUNT(*) AS num FROM {$table} WHERE auction_user='$_username' AND status=3 AND auction_status=1");
	$nums[6] = $rs['num']; 
}
if ($status == 5) {
	include template('my_'.$module.'_list', 'member');
}elseif($status == 6){
	include template('my_'.$module.'_list1', 'member');
}else{
