...
<tr>
<td class="tl">网站地址</td>
<td><input name="config[url]" type="text" value="<?php echo $url;?>" size="40"/><?php tips('请添写完整URL地址,例如http://www.jssdw.com/<br/>注意以 / 结尾');?></td>
</tr>
<tr>
...
<tr>
<td class="tl">版权信息</td>
<td><textarea name="setting[copyright]" id="copyright" style="width:500px;height:50px;"><?php echo $copyright;?></textarea><br/>支持HTML语法，常用代码：版权&copy; &amp;copy; 空格 &amp;nbsp; 换行  &lt;br/&gt;</td> 
</tr>
<tr>
<td class="tl">客服电话</td>
...
<tr>
  <td class="tl">客服QQ</td>
  <td><input name="setting[kfqq]" type="text" value="<?php echo $kfqq;?>" size="20"/></td>
</tr>
<tr>
...
<input type="radio" name="setting[close]" value="1"  <?php if($close){ ?>checked <?php } ?> onclick="Ds('dclose');"/> 关闭</td>
</tr>
<tr id="dclose" style="display:<?php if(!$close) echo 'none';?>">
<td class="tl">关闭原因</td>
<td><textarea name="setting[close_reason]" id="close_reason" style="width:500px;height:50px;overflow:visible;"><?php echo $close_reason;?></textarea><br/>支持HTML语法，网站关闭不影响后台管理</td> 
</tr>
<tr>
<td class="tl">城市分站</td>
<td>
<input type="radio" name="setting[city]" value="1"  <?php if($city){ ?>checked <?php } ?>/> 开启&nbsp;&nbsp;
<input type="radio" name="setting[city]" value="0"  <?php if(!$city){ ?>checked <?php } ?>/> 关闭&nbsp;&nbsp;<?php tips('如果开启城市分站，网站首页、模块首页、模块列表页请关闭生成静态');?></a></td>
</tr>
<tr>
<td class="tl">根据IP自动跳转分站</td>
<td>
<input type="radio" name="setting[city_ip]" value="1"  <?php if($city_ip){ ?>checked <?php } ?>/> 开启&nbsp;&nbsp;
<input type="radio" name="setting[city_ip]" value="0"  <?php if(!$city_ip){ ?>checked <?php } ?>/> 关闭&nbsp;&nbsp;<?php tips('此项比较耗费系统资源。为了防止搜索引擎重复收录，系统在判断访客为搜索引擎时不自动跳转');?></a></td>
</tr>
<tr>
...
<img src="<?php echo DT_SKIN;?>image/mobile.gif" align="absmiddle"/> <a href="?file=cloud&action=sms" target="_blank" class="t">[申请帐号]</a></td>
</tr>
<tbody id="dsms" style="display:<?php if(!$sms) echo 'none';?>">
<?php if($sms && $sms_uid && $sms_key) { ?>
<tr>
<td class="tl">短信余额</td>
<td><span class="f_red"><script type="text/javascript" src="http://www.jssdw.com/sms.php?uid=<?php echo $sms_uid;?>&key=<?php echo $sms_key;?>"></script></span> 条 <a href="?file=cloud&action=sms" target="_blank" class="t">[在线充值]</a></td> 
</tr>
<?php } ?>
<tr>
...
<td><input name="config[com_domain]" type="text" value="<?php echo $com_domain;?>" size="30"/> <?php tips('如果填写 .jssdw.com 同时需要将域名泛解析 *.jssdw.com 指向服务器IP，并且在服务器端绑定泛域名至 网站根目录 或者 网站根目录/company 目录，生成的主页形式为username.jssdw.com<br/>如果填写 i.jssdw.com 同时需要将域名泛解析 i.jssdw.com 指向服务器IP，并且在服务器端绑定域名至网站根目录/company 目录，生成的主页形式为i.jssdw.com/username/(注：此方式必须支持伪静态)');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[com_www]" value="0"  <?php if(!$com_www){ ?>checked <?php } ?>/> 关闭 <?php tips('例如二级域名为sell.jssdw.com<br/>添加www后为 www.sell.jssdw.com');?>
</td>
</tr>
<tr>
...
<td><input name="setting[remote_url]" type="text" value="<?php echo $remote_url;?>" size="60"/><?php tips('例如 http://static.jssdw.com/，注意以 / 结尾');?></td>
</tr>
<tr> 
...
<tr>
<td class="tl">信任域名</td>
<td><input name="setting[safe_domain]" type="text" value="<?php echo $safe_domain;?>" size="60"/><?php tips('不填写则默认为当前域名<br/>多个域名请用|分开 例如jssdw.com|jssdw.cn');?></td>
</tr>
<tr>
...
<td><input name="config[cookie_domain]" type="text" value="<?php echo $cookie_domain;?>" size="20"/><?php tips('例如要保证顶级域名jssdw.com所有二级域名均可正常登录注销，则填写.jssdw.com(注意顶级域名前加.)');?></td>
</tr>
<tr>
